...
WORKSPACE_DIR="${1:-$PWD}"
WORKSPACE_FILE="${STRUCTURIZR_WORKSPACE_FILE:-workspace.dsl}"
...
if [ ! -f "$WORKSPACE_DIR/$WORKSPACE_FILE" ]; then
  ...
  echo "Run this command from a directory containing $WORKSPACE_FILE, or pass the workspace directory as the first argument." >&2
  exit 1
fi
